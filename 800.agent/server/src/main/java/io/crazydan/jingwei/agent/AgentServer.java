...
import static io.crazydan.jingwei.agent.AgentServerConfigs.CFG_AGENT_API_TOKEN;
import static io.crazydan.jingwei.agent.AgentServerConfigs.CFG_AGENT_DATA_DIR;
import static io.crazydan.jingwei.agent.AgentServerConfigs.CFG_AGENT_SERVER_HOST;
import static io.crazydan.jingwei.agent.AgentServerConfigs.CFG_AGENT_SERVER_PORT;
...
public class AgentServer {
    ...
    public void start() {
        ...
        File dataDir = FileHelper.getAbsoluteFile(new File(CFG_AGENT_DATA_DIR.get()));
    ...
    private void checkConfigs() {
        if (CFG_AGENT_SERVER_PORT.get() == null) {
            throw new NopException(ERR_CFG_VALUE_NOT_SPECIFIED).param(ARG_NAME, CFG_AGENT_SERVER_PORT.getName());
        }
        if (StringHelper.isBlank(CFG_AGENT_DATA_DIR.get())) {
            throw new NopException(ERR_CFG_VALUE_NOT_SPECIFIED).param(ARG_NAME, CFG_AGENT_DATA_DIR.getName());
        }
    }
    ...
    private void prepareConfig(File dataDir) {
        ...
        config.put(KEY_HOST, CFG_AGENT_SERVER_HOST.get());
        config.put(KEY_PORT, CFG_AGENT_SERVER_PORT.get());
        config.put(KEY_TOKEN, CFG_AGENT_API_TOKEN.get());
